from vpython import sphere, vector, rate, sin, cos, box, label,color,arrow,vec
import numpy as np
...
slit_size = 0.5  # Half of the original size
gap_size = 0.5  # Space between the top and bottom walls

# Create the top and bottom parts of the wall around the smaller slits
top_wall = box(pos=wall_pos1 + vector(0 + wall_thickness/2, wall_height / 4 + gap_size / 2, 0), 
               #size=vector(wall_width, wall_height / 2 - slit_size, wall_thickness), color=vector(0.5, 0.5, 0.5))
               length=wall_width,
               height= wall_height / 2 - slit_size, 
               width=wall_thickness,
              color=vector(0.5, 0.5, 0.5))
bottom_wall = box(pos=wall_pos1 + vector(0 + wall_thickness/2, -wall_height / 4 - gap_size / 2, 0), 
                  #size=vector(wall_width, wall_height / 2 - slit_size, wall_thickness), color=vector(0.5, 0.5, 0.5))
                  length=wall_width,
                  height= wall_height / 2 - slit_size, 
                  width=wall_thickness,
...
def simulate_ball():
    ...
    if -slit_size / 2 < amplitude_shift*sin(0 + phase_shift) < slit_size / 2:
        pass
    else:
        #sine does not pass the walls
        return False
    ...
    while t < 4*30:
        ...
        y =  amplitude_shift* sin(t*freq + phase_shift) # y follows a sine curve with the random phase shift 
        if has_interacted:
            #rotate x and y with tangent angle
        ...
        if (0 < ball.pos.x < wall_width) and has_interacted ==False:
            if (-slit_size / 2 < ball.pos.y < slit_size / 2) or has_interacted:  # Ball passes through the smaller slits
                ...
                tangent = amplitude_shift*cos(t + phase_shift)  # dy/dx for y = sin(t + phase_shift) is cos(t + phase_shift)
                ...
                #a = arrow(pos=vector( ball.pos.x,  ball.pos.y, 0), axis=vec(tangent, 1, 0), color=color.orange)
                ...
                has_interacted = True
                print("Angle: " + str(math.degrees(angle)) + " tangent " + str(dy) +'coords: '+str(ball.pos) ) 
...
num_balls = 300
label(pos=vector(-7,4,0),text='Slit size: ' + str(slit_size),box=False ,height=10,color=vector(0,255,0))
...
plt.title("Distribution of Y-Positions at the Second Wall ")
